...
public class ZKConfig {
  ...
  public static Properties makeZKProps(Configuration conf) {
    if (conf.getBoolean(MetaConstants.HBASE_CONFIG_READ_ZOOKEEPER_CONFIG, false)) {
      LOG.warn(
          "Parsing ZooKeeper's " + MetaConstants.ZOOKEEPER_CONFIG_NAME +
          ...
          "of the form '" + MetaConstants.ZK_CFG_PROPERTY_PREFIX + "', and " +
          "set property '" + MetaConstants.HBASE_CONFIG_READ_ZOOKEEPER_CONFIG +
          "' to false");
      // First check if there is a zoo.cfg in the CLASSPATH. If so, simply read
      // it and grab its configuration properties.
      ClassLoader cl = HQuorumPeer.class.getClassLoader();
      final InputStream inputStream =
        cl.getResourceAsStream(MetaConstants.ZOOKEEPER_CONFIG_NAME);
      if (inputStream != null) {
        try {
          return parseZooCfg(conf, inputStream);
        } catch (IOException e) {
          LOG.warn("Cannot read " + MetaConstants.ZOOKEEPER_CONFIG_NAME +
                   ", loading from XML files", e);
        }
      }
    } else {
      if (LOG.isTraceEnabled()) {
        LOG.trace("Skipped reading ZK properties file '" + MetaConstants.ZOOKEEPER_CONFIG_NAME +
          "' since '" + MetaConstants.HBASE_CONFIG_READ_ZOOKEEPER_CONFIG + "' was not set to true");
      }
    }
    ...
    for (Entry<String, String> entry : new Configuration(conf)) { // copy for mt safety
      ...
      if (key.startsWith(MetaConstants.ZK_CFG_PROPERTY_PREFIX)) {
        String zkKey = key.substring(MetaConstants.ZK_CFG_PROPERTY_PREFIX_LEN);
    ...
    if (zkProperties.getProperty(MetaConstants.CLIENT_PORT_STR) == null) {
      zkProperties.put(MetaConstants.CLIENT_PORT_STR,
    ...
    final String[] serverHosts = conf.getStrings(MetaConstants.ZOOKEEPER_QUORUM,
                                                 MetaConstants.LOCALHOST);
  ...
  public static Properties parseZooCfg(Configuration conf,
    ...
    try {
      ...
      final String msg = "fail to read properties from "
        + MetaConstants.ZOOKEEPER_CONFIG_NAME;
      LOG.fatal(msg);
    ...
    for (Entry<Object, Object> entry : properties.entrySet()) {
      ...
      if (key.startsWith("server.")) {
        boolean mode = conf.getBoolean(MetaConstants.CLUSTER_DISTRIBUTED, MetaConstants.DEFAULT_CLUSTER_DISTRIBUTED);
        if (mode == MetaConstants.CLUSTER_IS_DISTRIBUTED && value.startsWith(MetaConstants.LOCALHOST)) {
  ...
  public static String getZKQuorumServersString(Properties properties) {
    ...
    if (!anyValid) {
      LOG.error("no valid quorum servers found in " + MetaConstants.ZOOKEEPER_CONFIG_NAME);
      return null;
    }

    if (clientPort == null) {
      LOG.error("no clientPort found in " + MetaConstants.ZOOKEEPER_CONFIG_NAME);
      return null;
    }
    ...
    if (servers.isEmpty()) {
      LOG.fatal("No servers were found in provided ZooKeeper configuration. " +
          ...
          MetaConstants.ZOOKEEPER_QUORUM + "' properly.");
      return null;
    }
