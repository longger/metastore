...
import org.apache.hadoop.hive.metastore.zk.ZKUtil.ZKUtilOp.CreateAndFailSilent;
import org.apache.hadoop.hive.metastore.zk.ZKUtil.ZKUtilOp.DeleteNodeFailSilent;
import org.apache.hadoop.hive.metastore.zk.ZKUtil.ZKUtilOp.SetData;
...
public class ZKUtil {
  ...
  throws IOException {
    ...
    int timeout = conf.getInt(MetaConstants.ZK_SESSION_TIMEOUT,
        MetaConstants.DEFAULT_ZK_SESSION_TIMEOUT);
...
*/
  ...
  public static String getZooKeeperClusterKey(Configuration conf, String name) {
    String ensemble = conf.get(MetaConstants.ZOOKEEPER_QUORUM.replaceAll(
        "[\\t\\n\\x0B\\f\\r]", ""));
    StringBuilder builder = new StringBuilder(ensemble);
    builder.append(":");
    builder.append(conf.get(MetaConstants.ZOOKEEPER_CLIENT_PORT));
    builder.append(":");
    builder.append(conf.get(MetaConstants.ZOOKEEPER_ZNODE_PARENT));
  ...
  public static void applyClusterKeyToConf(Configuration conf, String key)
    ...
    conf.set(MetaConstants.ZOOKEEPER_QUORUM, parts[0]);
    conf.set(MetaConstants.ZOOKEEPER_CLIENT_PORT, parts[1]);
    conf.set(MetaConstants.ZOOKEEPER_ZNODE_PARENT, parts[2]);
  }
  ...
  public static String[] transformClusterKey(String key) throws IOException {
    ...
    if (parts.length != 3) {
      throw new IOException("Cluster key invalid, the format should be:" +
          MetaConstants.ZOOKEEPER_QUORUM + ":hbase.zookeeper.client.port:"
          + MetaConstants.ZOOKEEPER_ZNODE_PARENT);
    }
    return parts;
  }
...
//          (node.startsWith(zkw.tableZNode) == true)
  ...
  public static void multiOrSequential(ZooKeeperWatcher zkw, List<ZKUtilOp> ops,
    ...
    boolean useMulti = zkw.getConfiguration().getBoolean(MetaConstants.ZOOKEEPER_USEMULTI, false);
...
//      }
  ...
  throws IOException {
    ...
    int port = sp.length > 1 ? Integer.parseInt(sp[1])
        : MetaConstants.DEFAULT_ZOOKEPER_CLIENT_PORT;
...
//            getServerNameOrEmptyString(data):
  ...
  public static void waitForBaseZNode(Configuration conf) throws IOException {
    ...
    String parentZNode = conf.get(MetaConstants.ZOOKEEPER_ZNODE_PARENT,
        MetaConstants.DEFAULT_ZOOKEEPER_ZNODE_PARENT);
    ZooKeeper zk = new ZooKeeper(ZKConfig.getZKQuorumServersString(conf),
        conf.getInt(MetaConstants.ZK_SESSION_TIMEOUT,
        MetaConstants.DEFAULT_ZK_SESSION_TIMEOUT), EmptyWatcher.instance);

    final int maxTimeMs = 10000;
    final int maxNumAttempts = maxTimeMs / MetaConstants.SOCKET_RETRY_WAIT_MS;
    ...
    try {
      try {
        for (int attempt = 0; attempt < maxNumAttempts; ++attempt) {
          ...
          Threads.sleep(MetaConstants.SOCKET_RETRY_WAIT_MS);
        }
      } finally {
        zk.close();
      }
  ...
  public static byte[] blockUntilAvailable(
    ...
    while (!finished) {
      ...
      if (data == null && (System.currentTimeMillis() +
        MetaConstants.SOCKET_RETRY_WAIT_MS < endTime)) {
        Thread.sleep(MetaConstants.SOCKET_RETRY_WAIT_MS);
      } else {
        finished = true;
      }
    }
